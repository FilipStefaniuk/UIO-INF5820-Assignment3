...
#SBATCH --job-name=emb-inf5820
#SBATCH --account=nn9447k
#SBATCH --time=00:30:00
#SBATCH --partition=accel
#SBATCH --gres=gpu:1
#SBATCH --mem-per-cpu=10000
#SBATCH --output=./logs/embeddings.log
...
for ((i=0; i < ${#WORD_VECTORS[@]}; i++)); do
    ...
    python train_model.py --mode=static --results_path="$OUTPUT_FILE" --word_vectors="${WORD_VECTORS[$i]}" --seed="$SEED" --model_tmp_path="emb.model.tmp"
done
