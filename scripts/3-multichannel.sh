...
#SBATCH --output=./results/baseline/multichannel/output.log
#SBATCH --err=./results/baseline/multichannel/err.log
...
for ((i=0; i < ${#WORD_VECTORS[@]}; i++)); do
    ...
    python train_model.py --mode=multichannel --results_path="$OUTPUT_FILE" --word_vectors="${WORD_VECTORS[$i]}" --seed="$SEED"
done