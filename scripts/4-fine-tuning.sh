...
#SBATCH --output=./results/baseline/fine_tuning/output.log
#SBATCH --err=./results/baseline/fine_tuning/err.log
...
for ((i=0; i < ${#WORD_VECTORS[@]}; i++)); do
    ...
    python train_model.py --mode=non-static --results_path="$OUTPUT_FILE" --word_vectors="${WORD_VECTORS[$i]}" --seed="$SEED"
done
